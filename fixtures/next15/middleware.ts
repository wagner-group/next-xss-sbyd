...
const enforce = withXssSbydHeaders(inner);
const reportOnly = withXssSbydHeaders(inner, {mode: "report-only"});
export const middleware = (request: NextRequest, event: Parameters<typeof enforce>[1]) =>
  request.nextUrl.searchParams.has("report-only") ? reportOnly(request, event) : enforce(request, event);
export const config = {matcher: ["/((?!api/vulnerable|static|_next/static|_next/image|favicon.ico).*)"]};
